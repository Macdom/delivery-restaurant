...
<head>
  ...
  <script src="../js/list_kurier.js" defer></script>
</head>

<body>
<br><br><br>
<center><h4>Kliknij w rząd, aby zmodyfikować lub usunąć kuriera.</h4></center>
<div class="container">
<center>
<?php
	...
	if($stat === PGSQL_CONNECTION_OK){
		...
		$result = pg_query("SELECT * FROM kurier ORDER BY id_kur");
		if (pg_num_rows($result) > 0) {
			...
			while($row = pg_fetch_assoc($result)) {
				...
				echo "<td>" . $row["id_kur"]. "</td><td>" . $row["imie"]. "</td><td>" . $row["nazwisko"]."</td><td>" . $row["placa"]."</td><td>" . $row["telefon"] . "</td>";
				echo "</tr>";
			}
			echo "</table>";
		} 
		else {
			echo "<strong>Brak kurierów.</strong>";
		}
	}
?>
<hr/>	
