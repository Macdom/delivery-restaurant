...
<head>
  ...
  <script src="../js/list_pojazd.js" defer></script>
</head>

<body>
<br><br><br>
<center><h4>Kliknij w rząd, aby zmodyfikować lub usunąć pojazd.</h4></center>
<div class="container">
<center>
<?php
	...
	if($stat === PGSQL_CONNECTION_OK){
		...
		$result = pg_query("SELECT * FROM pojazd ORDER BY id_poj");
		if (pg_num_rows($result) > 0) {
			echo "<table><tr class='nagl'><th>ID</th><th>model</th><th>cena</th></tr>";
			while($row = pg_fetch_assoc($result)) {
				$id = $row["id_poj"];
				echo "<tr class='element' onclick='Przekieruj($id)'>";		
				echo "<td>" . $row["id_poj"]. "</td><td>" . $row["model"]. "</td><td>" . $row["cena"]."</td>";
				echo "</tr>";
			}
			echo "</table>";
		} 
		else {
			echo "<strong>Brak pojazdów.</strong>";
		}
	}
?>
<hr/>	
