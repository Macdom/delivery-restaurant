...
<html lang="pl">
<head>
  <title>Strona logowania - klient</title>
...
<?php
	...
	if (isset($_POST['submit'])){
		...
		$result = pg_query("SELECT COUNT(*) FROM klient WHERE telefon = '$telefon'");
		$row = pg_fetch_row($result);
		echo $row[0];
		if($row[0] == 1) {
			$result2 = pg_query("SELECT id_kl FROM klient WHERE telefon = '$telefon'");
			$row2 = pg_fetch_row($result2);
			$_SESSION['klientID'] = $row2[0];
			$_SESSION['login'] = 1;
			$_SESSION['klient'] = 1;
			$logTest = 1;
		}
