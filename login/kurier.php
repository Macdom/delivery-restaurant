...
<html lang="pl">
<head>
  <title>Strona logowania - kurier</title>
...
<?php
	...
	if (isset($_POST['submit'])){
		...
		$result = pg_query("SELECT COUNT(*) FROM kurier WHERE telefon = '$telefon'");
		$row = pg_fetch_row($result);
		echo $row[0];
		if($row[0] == 1) {
			$result2 = pg_query("SELECT id_kur FROM kurier WHERE telefon = '$telefon'");
			$row2 = pg_fetch_row($result2);
			$_SESSION['kurierID'] = $row2[0];
			$_SESSION['login'] = 1;
			$_SESSION['kurier'] = 1;
			$logTest = 1;
		}
