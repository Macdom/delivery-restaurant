...
<html lang="pl">
<head>
  <title>Modyfikacja kucharza</title>
...
<?php
	...
	$kuch_query = pg_query("SELECT imie, nazwisko, placa, telefon FROM kucharz WHERE id_kuch = $id");
	$kuch_row = pg_fetch_assoc($kuch_query);
	$stare_imie = $kuch_row['imie'];
	$stare_nazwisko = $kuch_row['nazwisko'];
	$stara_placa = $kuch_row['placa'];
	$stary_telefon = $kuch_row['telefon'];
	...
	if (isset($_POST['zmien'])){
		...
		if ($valid = true){
			pg_query("UPDATE kucharz SET imie = '$imie', nazwisko = '$nazwisko', placa = '$placa', telefon = '$telefon' WHERE id_kuch = $id");
			header("Location: ../list/kucharz.php");
			exit();
		}
	}
	
	if (isset($_POST['usun'])){
		pg_query("DELETE FROM kucharz WHERE id_kuch = $id");
		header("Location: ../list/kucharz.php");
		exit();
	}
	}
...
?> 
	...
	<br><br><br>
		...
		<form enctype="multipart/form-data" name="form" id="form" method="post"
			...
			<div class="form-group row">
				<label for="imie" class="col-sm-4 col-form-label"><b>Imię kucharza:</b></label>
				<div class="col-sm-7">	
					<input type="text" class="form-control" id="imie" name="imie" value="<?php echo $stare_imie;?>" placeholder="Imię...">	
				</div>
			</div>	
			<div class="form-group row">
				<label for="nazwisko" class="col-sm-4 col-form-label"><b>Nazwisko kucharza:</b></label>
				<div class="col-sm-7">	
			...
			<div class="col-sm-4">
				...
				<div class="col-sm-7">
					...
					<input type="submit" id="usun" name="usun" class="btn btn-danger" value="Usuń kucharza">
					<hr/>
					<a class="btn btn-secondary" href="../list/kucharz.php">Powrót</a>	
				</div>
			</div>	
		</form>
