...
<html lang="pl">
<head>
  <title>Modyfikacja klienta</title>
...
<?php
	$stare_imie = $imie =  $stare_nazwisko = $nazwisko = $stary_telefon = $telefon = $imieErr = $nazwiskoErr = $placaErr = $telefonErr = "";
	...
	$kl_query = pg_query("SELECT imie, nazwisko, telefon FROM klient WHERE id_kl = $id");
	$kl_row = pg_fetch_assoc($kl_query);
	$stare_imie = $kl_row['imie'];
	$stare_nazwisko = $kl_row['nazwisko'];
	$stary_telefon = $kl_row['telefon'];
	...
	if (isset($_POST['zmien'])){
		...
		if ($valid = true){
			pg_query("UPDATE klient SET imie = '$imie', nazwisko = '$nazwisko', telefon = '$telefon' WHERE id_kl = $id");
	...
	if (isset($_POST['usun'])){
		pg_query("DELETE FROM klient WHERE id_kl = $id");
...
?> 
	...
	<div class="container">
	<br><br><br>
		<h1 class="display-4">Modyfikacja klienta</h1>
		...
		<form enctype="multipart/form-data" name="form" id="form" method="post"
			...
			<div class="col-sm-4">
				...
				<div class="col-sm-7">
					...
					<input type="submit" id="usun" name="usun" class="btn btn-danger" value="Usuń konto">
					<hr/>
